...
class Player(Entity):
    ...
    def input(self):
        if not self.attacking:
            ...
            self.status = "up" if keys[pygame.K_w] else "down" if keys[pygame.K_s] else self.status
            self.direction.x = 1 if keys[pygame.K_d] else -1 if keys[pygame.K_a] else 0
            self.status = "right" if keys[pygame.K_d] else "left" if keys[pygame.K_a] else self.status
